...
def sign_csr():
    ...
    if ( request.form['signer'] == u'client' ):
        ca = "client-CA"
        msg = u'Client'
    fname_cac = CApath + u"/" + ca + u".crt"
    fname_cak = CApath + u"/" + ca + u".key"
    fname_car = CApath + u"/" + u'cert.flow.info.crt'
    ...
    if not (os.path.isfile(fname_car)):
        print u"ERROR: Root certificate file ", fname_car, " does not exist"
        runOk = False

    if not (os.path.isfile(fname_cac)):
        print u"ERROR: CA certificate file ", fname_cac, " does not exist"
        runOk = False

    if not (os.path.isfile(fname_cak)):
        print u"ERROR: CA key file ", fname_cak, " does not exist"
        runOk = False
